...
computeAvgPos(sheeps){
  ...
  for(let other of sheeps){
    //if(!this.isInMyView(other.pos, 360, this.perceptions[this.cohe]))
      //continue;
...
computeAlignSteer(sheeps){
  let desired = this.computeAlignDesired(sheeps);
...
computeFleeDesired(direction, magnitude){
  ...
  if(magnitude <= this.flee_min_perception){
    ...
    m = map(magnitude, this.flee_min_perception, this.perceptions[this.flee], this.speed_limits[this.flee], 0);
  }
...
computeFleeSteer(direction, magnitude){
  let desired = this.computeFleeDesired(direction, magnitude);
...
computeBehaviours(sheeps){
  ...
  this.accelerations[this.flee] = this.computeFleeSteer(v, m).mult(fleeSlider.value());

  this.accelerations[this.cohe] = this.computeCoheSteer(neighbors);

  this.accelerations[this.align] = this.computeAlignSteer(neighbors).mult(alignSlider.value());
